...
export function Img(
  props: ImagePropsWithOptionalAlt & {
    placeholder?: string;
  },
) {
  return (
    <div className="relative mt-8 overflow-hidden rounded-xl bg-card [&+*]:mt-8">
      <Image
        alt=""
...
export function ContentWrapper({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return (
...
export function ArticleHeader({ id, date }: { id: string; date: string | Date }) {
  return (
    <header className="relative mb-10 xl:mb-0">
      <div className="pointer-events-none absolute left-[max(-0.5rem,calc(50%-18.625rem))] top-0 z-50 flex h-4 items-center justify-end gap-x-2 lg:left-0 lg:right-[calc(max(2rem,50%-38rem)+40rem)] lg:min-w-[32rem] xl:h-8">
        ...
        <div className="h-[0.0625rem] w-3.5 bg-secondary lg:-mr-3.5 xl:mr-0" />
      </div>
      <ContentWrapper>
        <div className="flex">
          <Link href={`#${id}`} className="inline-flex">
            <FormattedDate
              date={date}
              className="text-2xs/4 font-medium text-foreground/50 xl:hidden"
            />
          </Link>
        </div>
      </ContentWrapper>
